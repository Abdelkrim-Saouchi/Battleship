...
test('handling invalid first index and last index works', () => {
  ...
  expect(() => gameBoardObj.placeShipAt(1, 'ab', 5, 'currier')).toThrow(
    'Invalid ship lastIndex'
  );
});
...
test('prevent placing new ship on existed ship', () => {
  const gameBoardObj = gameBoardFactory();
  gameBoardObj.placeShipAt(0, 4, 5, 'currier'); // old ship
  gameBoardObj.placeShipAt(0, 3, 4, 'BattleShip'); // new ship
  expect(gameBoardObj.gameBoard[0][0]).toBe('currier');
});

test('receiveAttack function works correctly', () => {
  const gameBoardObj = gameBoardFactory();
  gameBoardObj.placeShipAt(0, 4, 5, 'currier'); // ship placed at first index 0 to last index 4 with length of 5
...
test('allAreSunk works correctly', () => {
  const gameBoardObj = gameBoardFactory();
  gameBoardObj.placeShipAt(0, 1, 1, 'submarine');
  gameBoardObj.receiveAttack(0, 0);
  expect(gameBoardObj.allAreSunk()).toBe(true);
  gameBoardObj.placeShipAt(1, 2, 3, 'battleShip');
